...
AS
    ...
    BEGIN
        ...
        FOREACH n IN ARRAY networks LOOP --72 networks
            ...
            CALL save_query_simple(original_query, query, results, md5(stations::varchar), download);
            ...
            IF array_length(stations, 1) > 0 THEN
                FOREACH s IN ARRAY stations LOOP
                    ...
                    CALL save_query_simple(original_query, query, results, md5(timeseries::varchar), download);
                    ...
                    FOREACH s IN ARRAY timeseries  LOOP
                        ...
                        CALL save_query_simple(original_query, query, results, md5(que::varchar), download);
                    END LOOP;
                    END IF;
                END LOOP;
            END IF;
