...
BEGIN
    ...
    EXECUTE format('INSERT INTO %s_hist SELECT * FROM %s;', tablename_wo_schema, tablename);
    ...
    SELECT STRING_agg(cn.pk, ' AND ')INTO primary_key_conditions_new FROM (SELECT CONCAT(column_name, '=NEW.',column_name)as pk FROM primary_keys WHERE table_name like tablename_wo_schema) as cn;
